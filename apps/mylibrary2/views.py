...
# PREVENTS ERROR: This section comes into play if no Book objects exist
        ...
        all_reviews = Review.objects.count() # added
        print("Review count is", all_reviews) # added
        # if all_reviews == 0 # added
            # latest3_reviews = {} # added
        # else: # added
...
# PREVENTS ERROR: this section comes into play when there are fewer than 3 Review objects
        all_reviews = Review.objects.all() # added
        review_count = all_reviews.count() # added
        if review_count < 3: # added
# display the latest 1 or 2 reviews and any existing books
            if review_count == 2:
                two_reviews = Review.objects.order_by("-created_at")[:2]
                context = {
                    'books': all_books,
                    'reviews': two_reviews,
                    'books_not_reviewed': books_not_reviewed, #empty array
                }
            elif review_count == 1:
                one_review = Review.objects.all()
                context = {
                    'books': all_books,
                    'reviews': one_review,
                    'books_not_reviewed': books_not_reviewed, # empty array
                }
            else: # review_count is zero
                no_reviews = {}
                context = {
                    'books': all_books,
                    'reviews': reviews, # empty dictionary
                    'books_not_reviewed': books_not_reviewed, # empty array
                }
            return render(request, 'mylibrary2/books.html', context)
